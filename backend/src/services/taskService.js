...
class TaskService {
  ...
  async createTaskFromData({ intent, entities, userContext = {}, userInput = '' }) {
    ...
    // attach creator if provided in userContext
    if (userContext && userContext.userId) {
      taskRow.created_by = userContext.userId;
    }
  ...
  async getAllTasks(userId) {
    // If userId provided, repository will filter by created_by
    return repo.getAllTasks(userId);
  }

  async getTaskById(taskId, userId) {
    const result = await repo.getTaskById(taskId, userId);
  ...
  async updateTaskStatus(taskId, newStatus, userId) {
    ...
    const current = await repo.getTaskById(taskId, userId);
    const oldStatus = current.task.status;
    const updated = await repo.updateTaskStatus(taskId, newStatus, userId);
